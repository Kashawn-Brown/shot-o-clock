...
Phase 11B, "Host-only single-phone mode," is complete and device-verified. A host can now run the whole game alone on one phone, with no other devices joining — the host's screen becomes the shared display and caller. It's driven by a single new `host_only` flag on the party's settings: the rest of the server is untouched, so single-phone is just a one-host party riding the same timer and round functions as a normal game, with the app hiding the multi-device surfaces. A new Single-Phone Mode toggle on Create Party turns it on; flipping it on also hides the elimination and grace controls, because single-phone forces elimination off — the host is the only player and never taps Done, so under elimination they would eliminate themselves and stall the game, whereas with it off the loop runs cleanly until they choose to end it. Creating a single-phone party skips the lobby entirely and drops straight into the timer.

Inside the game the player-only surfaces fall away: the timer loses its Players sheet, I'm Out button, join-code popover, and results link, and the full-screen Shot O'Clock moment keeps its drama but drops Done / I'm Out, looping straight back to the next countdown instead of stopping at Round Results. Ending the party lands on a minimal end screen showing only rounds played and total time elapsed — there are no per-player outcomes to rank — served by the existing summary screen branching on the flag, so every "party ended" route reaches it for free. The whole mode is additive: no game logic changed, and the multi-device path behaves exactly as before. Next is Phase 12, the hardening and cleanup pass that closes out the MVP.

...
Phase 11, "Final Summary," is complete and device-verified. When the host ends a party — from the timer's End Party control or from the zero-active halt on Round Results — every device now lands on a real game-over screen instead of being dropped back home. The Final Summary is a whole-game wrap-up rather than a repeat of the last round: it reads the end-of-game roster once and ranks everyone by cumulative shots taken, crowning the shot leader(s) — "Last Standing" with elimination on, "Top Shots" with it off, showing names for up to three tied players and a head-count beyond that — alongside the total rounds played and a ranked standings list, with kicked players greyed at the bottom and never crowned. The screen is terminal and read-only, so no game state can change from it, and "Back to Home" resets the navigation stack. The ranking lives in a pure, unit-tested derivation over the `get_party_state` read the app already had, so no new server function or migration was needed.

The routing was split cleanly along intent. The host's End Party is the deliberate wrap-up and now routes everyone to the summary — the host directly, the other devices following through the realtime "party ended" detection already in place — while the guest's Leave Party escape hatch and a removed player still route home, since neither is ending the game. The halt's End Party also lost its confirmation prompt: when everyone is already out, ending is the only logical next step, so it ends straight to the summary on one tap. Next is Phase 11B, host-only single-phone mode, with Phase 12 hardening behind it.

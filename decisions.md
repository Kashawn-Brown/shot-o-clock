...
- **#006:** Reconnect (launch-time active-party detection + routing) is built but verified only at the unit level (phase→route mapping). End-to-end verification — force-close mid-party and reopen — needs a real party to exist, so it defers to Phase 5 (create/join wiring). _Update 2026-06-08:_ the `getActiveParty` launch read surfaced the RLS recursion bug fixed in D020; the `party_sessions` SELECT policy now returns the caller's own party as assumed (verified at the DB level). Remaining: the on-device force-close-and-reopen walk, once a party can be created on device in Phase 5.

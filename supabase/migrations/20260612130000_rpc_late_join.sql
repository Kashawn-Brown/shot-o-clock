-- supabase/migrations/20260612130000_rpc_late_join.sql
-- ─────────────────────────────────────────────────────────────────────────────
-- Phase 10B — mid-game joining (D021 / D042): a FRESH player joins a party that
-- has already started. Distinct from reconnect (§3.6) and self-left rejoin
-- (D024) — both of those are returning members; this is a brand-new row.
--
-- Entry model (decided 2026-06-12): ACTIVE NEXT ROUND. The joiner lands as an
-- active member immediately (roster syncs live, routed by current phase), but is
-- excluded from the finalization of the round they joined during — so the
-- in-flight round's active count / elimination math is untouched (no off-by-one).
-- Their shot tally starts at the round they joined; grace applies from their
-- first active round forward (D021).
--
-- Four additive pieces, all idempotent on a fresh DB:
--   1. party_players.joined_round_number — the round a late joiner came in at
--      (NULL = joined in lobby / before the game). Mirrors out_round_number /
--      used_grace_round_number.
--   2. party_settings.allow_late_join default → true (MVP default for new
--      parties; existing rows unaffected; schema default in initial_schema stays
--      false — this is the additive override).
--   3. join_party — relax the §3.4 new-join lobby precondition: a live running
--      party (status active/paused, phase countdown/shot_window) with late join
--      on accepts a fresh joiner and stamps joined_round_number. Ended parties,
--      late-join-off, and the zero-active round_complete HALT stay blocked.
--   4. finalize_round_outcomes — exclude the current round's joiners from the
--      per-player finalization loop (the only change vs. the D034 version).
--
-- Locked conventions (#D010): SECURITY DEFINER, SET search_path = public,
-- pg_temp; finalize's REVOKE EXECUTE re-asserted at the bottom.
-- References: docs/specs/rpc-contracts.md §3.4–§3.6, decisions.md D021/D042/D034.
-- ─────────────────────────────────────────────────────────────────────────────


-- ─── 1. Track the round a late joiner came in at ─────────────────────────────
alter table party_players
  add column if not exists joined_round_number int
    check (joined_round_number is null or joined_round_number >= 1);

comment on column party_players.joined_round_number is
  'Round number during which this player joined a running party (late join, '
  'D021/D042). NULL = joined in lobby / before the game started. The player is '
  'excluded from the finalization of the exact round they joined — active NEXT '
  'round, not this one.';


-- ─── 2. allow_late_join default → true for new parties (Phase 10B MVP) ───────
-- create_party omits allow_late_join, so it inherits this default. The
-- initial_schema column default (false) is left intact; this is the additive
-- override so MVP parties accept mid-game joiners. Existing rows are unchanged.
alter table party_settings
  alter column allow_late_join set default true;


-- ─── 3. join_party — lobby join, reconnect, self-left rejoin, OR late join ────
-- Rebuilt from 20260608130000 (self-left rejoin / D024). The reconnect (§3.6)
-- and self-left branches are byte-for-byte the prior behavior except each return
-- payload now also carries is_late_join (always false for those paths). The
-- new-join path is where Phase 10B lands: lobby → ordinary pre-game join; a live
-- running party with late join on → active-next-round joiner.
...
begin
  ...
  -- ─── New-join path (§3.4 + Phase 10B late join, D021/D042) ──────────────
  ...
  --   on: the joiner comes in active NEXT round — joined_round_number = the
  --   current round, which finalize_round_outcomes excludes from THIS round's
  --   scoring, so active counts / elimination are untouched.
...
comment on function public.join_party(text, text) is
  ...
  'party mid-game (Phase 10B late join, active next round — D021/D042). '
  'Host-kicked callers get PLAYER_REMOVED. See docs/specs/rpc-contracts.md §3.';


-- ─── 4. finalize_round_outcomes — skip the round's late joiners ───────────────
-- Rebuilt from 20260611120000 (D034 grace-aware self_out). The ONLY change is
-- the per-player loop's WHERE: a player whose joined_round_number equals the
-- round being finalized joined DURING this round and is excluded — they are
-- active NEXT round, not scored on the one they walked into. Every other
-- statement (the grace/skip ladders, the upsert, the player-row update, the
-- round-completed update + event, the advance_to_next_round delegation) is
...
begin
  ...
  -- Phase 10B: a player whose joined_round_number = this round joined mid-round
  -- and is excluded — active NEXT round, not scored on the one they walked into
  -- (no off-by-one in active count / elimination). NULL (pre-game members) and
  -- earlier joiners are always included.
  for v_player in
    select id, used_grace
    from party_players
    where party_session_id = p_party_session_id
      and status = 'active'
      and coalesce(joined_round_number, 0) < v_round_number
  loop
...
comment on function public.finalize_round_outcomes(uuid, triggered_by, uuid) is
  ...
  'grace-aware skip per D034; mid-round late joiners are excluded per D021/D042), '
  'then delegates to advance_to_next_round for the D014 auto-advance. EXECUTE '
  'revoked from anon/authenticated; only advance_phase_if_due and '
